...
public class MainSetup implements Setup {
    ...
    public void init(NutConfig config) {
        try {
            ...
            // 初始化数据表
            //initSysData(config, dao);//TODO 需要自动建表和插入初始化数据是打开
    ...
    private void initSysData(NutConfig config, Dao dao) {
        ...
        if (0 == dao.count(Sys_user.class)) {
            ...
            menu.setName("配置项管理");
            menu.setAliasName("DictConfig");
            menu.setLocation(0);
            menu.setHref("/private/sys/dictConfig");
            menu.setShow(true);
            menu.setPermission("sys:dictConfig");
            menu.setParentId(m1.getId());
            menu.setType("menu");
            Sys_menu m10 = dao.insert(menu);
            //初始化角色
            ...
            dao.execute(Sqls.create("insert into `sys_role_menu` (`role_id`, `menu_id`) values('" + dbrole.getId() + "','" + m10.getId() + "')"));
            //另外一种写法(安全)
            ...
            //初始化配置项数据
            dao.execute(Sqls.create("INSERT INTO `dict_config` (`code`, `name`, `key`, `value`, `note`) VALUES (@a,@b,@c,@d,@e)")//
                    .setParam("a", "common")//
                    .setParam("b", "常规配置")//
